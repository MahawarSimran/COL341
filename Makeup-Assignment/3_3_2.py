...
def initialize_model():
    ...
    nn.Linear(25088, 4096),  # Replace 25088 with the number of input features to the fully connected layer
    ...
    nn.ReLU(inplace=True),
    nn.Dropout(),
    nn.Linear(4096, 1)  # Replace 4096 with the number of output features for regression
)
    return base_model
...
class Model:
    ...
    def train(self, to_train, Y):
        ...
        for epoch in range(self.epochs):
            ...
            test_loss = compute_test_loss(float(dataloader, model, Y, criterion, device))
